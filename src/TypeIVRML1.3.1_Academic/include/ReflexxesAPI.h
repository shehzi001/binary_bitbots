...
//!
//! \details
//! The class ReflexxesAPI is the interface between the actual Online 
...
//  ---------------------- Doxygen info ----------------------
//! \fn ReflexxesAPI(const unsigned int &DegreesOfFreedom, const double &CycleTimeInSeconds, const unsigned int &NumberOfAdditionalThreads = 0, const double &MaxTimeForOverrideFilterInSeconds = 2.0)
//!
//! \brief
...
//! velocity-based (TypeIVRMLVelocity) Online Trajectory Generation.
//! 
//! \warning
...
//! Specifies the cycle time of the Online Trajectory Algorithms in seconds
//! ReflexxesAPI::CycleTime
//!
//! \param NumberOfAdditionalThreads
//! \if RMLTYPEIV It can be reasonable to parallelize the position-based Online Trajectory
...
//! \endif 
//!
//! \param MaxTimeForOverrideFilterInSeconds
//! Specifies the filter time of the moving average filter for override
//! input values. The default value is 2.0 seconds.
//! 
//! \if CUSTOMER
//! \note
//! The automatic creation of additional threads is disabled by default. In
...
//  ----------------------------------------------------------
    ReflexxesAPI(		const unsigned int	&DegreesOfFreedom
                    ...
                    ,	const double		&MaxTimeForOverrideFilterInSeconds	=	2.0);
...
//! velocity-based Online Trajectory Generation. If the constructor 
...
//! 
//! \brief
//! Result values of the Online Trajectory Generation algorithm
//! 
//! \details
...
//  ----------------------------------------------------------
    ...
    {
        //! \details
        //! The Online Trajectory Generation algorithm is working; the final
		...
		//! ReflexxesAPI::SetupOverrideFilter() if executed
		//! successfully.
        RML_NO_ERROR							=	2,
		//! \details
		...
		//! \details
		//! If the flag RMLFlags::PositionalLimitsBehavior is set to
		//! <ul>
		//!   <li>RMLFlags::POSITIONAL_LIMITS_ERROR_MSG_ONLY or</li>
		//!   <li>RMLFlags::POSITIONAL_LIMITS_ACTIVELY_PREVENT</li>
		//! </ul>
		//! the methods ReflexxesAPI::RMLPosition() and
		//! ReflexxesAPI::RMLVelocity() check whether the currently 
		//! computed trajectory is within the positional limits
		//! \f$ \vec{P}_i^{\,max} \f$ or \f$ \vec{P}_i^{\,min} \f$. If
		//! these limits are exceeded, this error value will be returned.
		RML_ERROR_POSITIONAL_LIMITS				=	-108,
		//! \details
		//! This is return value indicates that the specified override value
		//! or filter time value is out of range.
		RML_ERROR_OVERRIDE_OUT_OF_RANGE			=	-109
    };
...
//!
//! \brief
//! This is the <b>central method</b> of each Reflexxes Type Motion Library
//!
//! \details
//! This method executes the position-based Reflexxes Online
...
//!
//! The coefficients of the position, velocity, and acceleration 
//! polynomials RMLOutputParameters::Polynomials including the time values
//! that determine the time intervals, within which the polynomials are
//! valid, are not computed w.r.t. the input value \c TimeValueInSeconds
//! but w.r.t. the time of the last call of ReflexxesAPI::RMLPosition().
//!
//! \note
//! This method only samples the trajectory, so that the override value is
//! applied here. The trajectory is sampled with an override value of 1.0
//! (100 %).
//!
//! \sa ReflexxesAPI::RMLPosition()
...
//!
//! \brief
//! This is the method for velocity-based Online Trajectory Generation.
//!
//! \details
//! This method executes the velocity-based Reflexxes Online
//! Trajectory Generation algorithm.\n
//! \n
...
//  ---------------------- Doxygen info ----------------------
//! \fn int SetupOverrideFilter(const double &FilterValue, const double &FilterTimeInSeconds)
//!
//! \brief
//! Sets up the override filter (initial value and filter time)
//!
//! \warning
//! This function resets the moving average filter for override values. If
//! override is active and the system is in motion, unsteady set points
//! will be provided due to the filter reset. <b>This method must only be
//! called during system initialization or during standstill.</b>
//!
//! \details
//! During common operations, the applied override value is not allowed to
//! change abruptly, and a moving average filter with linear intermediate 
//! segments is applied in order to obtain smooth transitions from an 
//! existing override value to a new desired override value. This method
//! initializes the filter and sets the filter time.
//! 
//! The default (i.e., the initial) override value is 1.0. In order to start
//! motions with a user-specified override value, this method can be called
//! right after the constructor ReflexxesAPI::ReflexxesAPI().
//!
//! \param FilterValue
//! The value to initialize the filter. Its valid range is 0 to 10, whereas
//! 0 will lead to standstill and 10 will be a speedup of 1000 percent.
//!
//! \param FilterTimeInSeconds
//! Filter time for the moving average filter with intermediate linear
//! segments. It can range between 0 and the value that was used to specify
//!	ReflexxesAPI::MaxTimeForOverrideFilter in the constructor
//! ReflexxesAPI::ReflexxesAPI(). The default time value is one second.
//!
//! \return
//!  - ReflexxesAPI::RML_NO_ERROR: \copydoc ReflexxesAPI::RML_NO_ERROR
//!  - ReflexxesAPI::RML_ERROR_OVERRIDE_OUT_OF_RANGE: \copydoc ReflexxesAPI::RML_ERROR_OVERRIDE_OUT_OF_RANGE\n
//!
...
//! \sa \ref page_Code_13_RMLPositionSampleApplication
//! \sa \ref page_Code_14_RMLVelocitySampleApplication
//  ----------------------------------------------------------
	int	SetupOverrideFilter(		const double &FilterValue
								,	const double &FilterTimeInSeconds	);
...
//  ---------------------- Doxygen info ----------------------
//! \var double MaxTimeForOverrideFilter
//!
//! \brief
//! Maximum time in seconds for the moving average filter of the override
//! input value
//!
//! \sa ReflexxesAPI::ReflexxesAPI()
//! \if CUSTOMER \sa TypeIVRMLPosition::MaxTimeForOverrideFilter \endif
//! \if CUSTOMER \sa TypeIVRMLVelocity::MaxTimeForOverrideFilter \endif
//! \sa \ref page_Override
//  ----------------------------------------------------------
...
//!
//! \brief
//! A pointer to an object for position-based Online Trajectory Generation
//!
//! \details
//! Depending on the type of Reflexxes Motion Library, an object containing
//! the functionality for the position-based Online Trajectory Generation
...
//!
//! \brief
//! A pointer to an object for velocity-based Online Trajectory Generation
//!
//! \details
//! Depending on the type of Reflexxes Motion Library, an object containing
//! the functionality for the velocity-based Online Trajectory Generation
//! algorithm is created by the constructor ReflexxesAPI::ReflexxesAPI().
