...
//! 
//! \brief
//! Class for the input parameters of the position-based Online
...
//! Optional pointer to an integer value. If the input values are not valid
//! (result values: \c false), the index of the degree of freedom, for
//! which the \b first error is detected will be written into this variable.
//! If the error is not related to a single degree of freedom, or if the
//! input values are valid, the value of this variable will not change.
//!
//! \param ErrorCode
//! This optional parameter is a pointer to an integer variable. If the
//! input values are valid (result value: \c true), a value of
//! RMLInputParameters::IP_NO_ERROR will be written into this variable.
//! If the input values are not valid (result values: \c false), an error
//! code from the enumeration 
//! RMLInputParameters::ErrorCodeForInvalidInputValues will be written into
//! this variable.
//!
//! \return
...
//! 
//! \note
//!  - In order to achieve an optimal performance of the Online Trajectory
...
//!    valid for the Online Trajectory Generation algorithm.\n\n
//!  - If the Online Trajectory Generation algorithm is fed with invalid
...
//! \sa RMLInputParameters::ErrorCodeForInvalidInputValues
//! \sa \ref page_InputValues
//  ----------------------------------------------------------
        ...
        for (i = 0; i < this->NumberOfDOFs; i++)
        {
            if ((this->SelectionVector->VecData)[i])
            {
                if (this->MaxVelocityVector->VecData[i]	<=	0.0)
                {
					...
					{
						*DegreeOfFreedom	=	i;
						*ErrorCode			=	RMLInputParameters::IP_MAX_VELOCITY;
					}
                    return(false);
                }
                      ...
                      >	(this->MaxVelocityVector->VecData)			[i]	)
				{
					...
					{
						*DegreeOfFreedom	=	i;
						*ErrorCode			=	RMLInputParameters::IP_TARGET_VELOCITY;
					}
					return(false);
				}								                
...
//! layer applies the velocity-based Online Trajectory Generation
...
//! for the velocity-based Online Trajectory Generation algorithm.\n\n
//!
//! This attribute can be accessed directly or by using one of the following methods:\n\n
...
//! position-based Online Trajectory Generation algorithm) a 
